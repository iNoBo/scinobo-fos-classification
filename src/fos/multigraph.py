...
- export_gephi(filename="multigraph.gexf"): Export the graph to Gephi format.
...
class MultiGraph(nx.MultiDiGraph):
    ...
    Methods:
        ...
        export_gephi(filename="multigraph.gexf"): Exports the graph to Gephi format.
    ...
    def add_entities(self, from_entities, to_entities, relationship_type, relationships, cutoff=0):
        """
            Add entities and relationships to the multigraph.
    ...
    def load(self, path):
        """
        Load a serialized multigraph from a file.

        Parameters:
        path (str): The path to the file containing the serialized multigraph.
